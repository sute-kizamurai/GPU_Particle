...
ID3D11BlendState*		Renderer::m_BlendStateAdd{};//加算合成ブレンドステート
...
void Renderer::Init()
{
	...
	m_DeviceContext->GSSetConstantBuffers(5, 1, &m_CameraBuffer);
...
void Renderer::CreateStructuredBuffer(int ElementSize, int ElementAmount, D3D11_SUBRESOURCE_DATA* sd, ID3D11Buffer** OutputBuffer)
{
	D3D11_BUFFER_DESC bd;
	bd.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	bd.ByteWidth = ElementSize * ElementAmount;
	...
	if (sd == nullptr)
	{
		...
		initData.pSysMem = nullptr; // 空のバッファを作成するためデータを指定しない
		HRESULT hr = m_Device->CreateBuffer(&bd, &initData, OutputBuffer);
		if(FAILED(hr))
			bd.Usage = D3D11_USAGE_DEFAULT;
	}
	else
	{
		m_Device->CreateBuffer(&bd, sd, OutputBuffer);
	}
...
void Renderer::CreateBufferSRV(ID3D11Buffer* Buffer, ID3D11ShaderResourceView* ShaderResourceView)
{
	...
	D3D11_BUFFER_DESC* bd{};
	Buffer->GetDesc(bd);
	srv.Buffer.NumElements = bd->ByteWidth / bd->StructureByteStride;

	m_Device->CreateShaderResourceView(Buffer, &srv, &ShaderResourceView);
}

void Renderer::CreateBufferUAV(ID3D11Buffer* Buffer, ID3D11UnorderedAccessView* UnorderedAccessView)
{
	...
	D3D11_BUFFER_DESC* bd{};
	Buffer->GetDesc(bd);
	uav.Buffer.NumElements = bd->ByteWidth / bd->StructureByteStride;

	m_Device->CreateUnorderedAccessView(Buffer, &uav, &UnorderedAccessView);
}
